...
|--------|-----------------|
| `FEATURE:Name` | 1. `// WEBSITE:FEATURE:Name` in code → 2. `docs/website/FEATURE-Name.md` → 3. Config file |
...
| `FAQ:Name` | 1. `// WEBSITE:FAQ:Name` in code → 2. `docs/website/FAQ-Name.md` |
...
| `PRICING` | StoreKit config or `.claude/website-content.yaml` |

#### Extraction Process

1. **Search code for markers**:
   ```bash
   grep -r "// WEBSITE:" --include="*.swift" [appPath]/Sources/
   ```

2. **Parse marker blocks**: Extract content between `// WEBSITE:TYPE:Name` and `// /WEBSITE`

3. **Check markdown fallback**: If no code marker, look for `docs/website/TYPE-Name.md`

4. **Check config fallback**: For structured content, check `.claude/website-content.yaml`

5. **Format for HTML**: Convert markdown to HTML, wrap in appropriate tags based on marker type
...
| "Features" or bullet lists under feature headings | `SYNC:FEATURES:SectionName` |
...
1. **Code comments** — `// WEBSITE:TYPE:Name` blocks in Swift files
2. **Markdown docs** — `docs/website/TYPE-Name.md` files
3. **Config files** — `website-content.json` or `.claude/website-content.yaml`
4. **Localization** — `Localizable.strings` entries prefixed with `WEBSITE_`
